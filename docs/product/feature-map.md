...
- Inline manuscript formatting is kept separate from render-only decorations: browser-slice author formatting is persisted with scene records, while the architecture target promotes it to canonical manuscript marks and keeps spellcheck, AI proposals, hover previews, and narration tracking as projected visual channels.
