...
- Scrivener project integrator for local `.scriv` packages, including a host-backed import route, browser path input, whitespace-preserving manuscript conversion, template retention, saved-project library merging, file-backed Save As/load support, and desktop-style resizable side panes.
- Scrivener import planning for the Project Serva Vitae source package, including the import command that now emits manuscript chapters/scenes, Scrivener comments as tasks, worldbuilding documents, nested station/fauna sheets, preserved Scrivener template sheets, timeline records, media-aware research notes with source provenance, full binder-path logging, and a saved-project seed that can be loaded, saved, and recreated from the browser UI.
