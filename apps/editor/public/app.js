...
function renderWritingTargetStrip(summary) {
...
function buildSessionTrackerMetric(summary) {
  if (!summary) {
    return {
      key: "sessionTracker",
...
function getSessionTrackerVisualState(summary, tracker) {
  const paceRatio = Number(summary?.sessionWordsPerMinuteRatio ?? tracker.progress ?? 0);
  ...
  if (summary?.sessionWordsPerMinuteOverTarget || paceRatio >= 1) {
    return { key: "flaming" };
  }
...
function renderSessionTrackerPaceRing(ratio, color) {
...
function renderSessionTrackerPanel(summary) {
  ...
  return `
    <article class="session-tracker-panel">
      <div class="session-tracker-panel__header">
        <div class="session-tracker-panel__brand">
          <span class="session-tracker-panel__badge" aria-hidden="true">
            <span class="session-tracker-panel__badge-icon">${renderSessionTrackerPenSvg(visualState.key)}</span>
          </span>
      ...
      <div class="session-tracker-panel__body">
        <div class="session-tracker-panel__progress-column">
          <div class="session-tracker-panel__bar" style="--writing-target-bar-color: ${paceColor};" aria-hidden="true">
        ...
        <div class="session-tracker-panel__gauge-column">
          <div class="session-tracker-panel__gauge ${visualState.key === "flaming" ? "is-over-target" : ""}">
            ${renderSessionTrackerPaceRing(paceRatio, paceColor)}
            <span class="session-tracker-panel__gauge-inner" aria-hidden="true">
          ...
          <div class="session-tracker-panel__gauge-copy">
            <strong>Words/min tracker</strong>
            <span>${escapeHtml(paceStatus)}</span>
          </div>
        </div>
      </div>
...
function renderSessionTrackerClockIcon() {
  return `
...
function renderWritingTargetCard(metric, summary) {
