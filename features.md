...
- Execution flow: editor boot loads base and reference word lists -> the active project lexicon is built from manuscript, binder, passage-note, dictionary, and exception text -> typing schedules a lightweight underline-layer refresh while excluding the active word -> right-clicking a flagged word or grammar-panel entry opens a spellcheck menu -> applying a suggestion edits the scene draft at known offsets, while dictionary/exception actions update project spellcheck settings
...
- Execution flow: Ctrl+F or the panel action opens the find window using the current selected text where available -> query updates recompute matches across scene editor text -> match navigation selects and centers the matched scene range -> replace current or replace all mutates scene drafts through `updateSceneDraft` with explicit find/replace reasons
...
- Execution flow: author selects manuscript text and opens the context menu -> task creation or inspiration/research note creation captures scene id, offsets, selected text, title, and source type -> inline note drafts can either reuse the selected range or insert typed verse into the manuscript before saving -> side-panel items navigate back to matched scene ranges with fuzzy drift recovery
...
- Execution flow: editor selection changes capture scene, block, offsets, line number, and viewport -> navigation requests select the scene and restore caret/scroll to the intended span -> layout sync sizes the textarea, gutter, spellcheck layer, and line metrics -> whitespace clicks move focus into the nearest editor position instead of leaving the author stranded outside the writing surface
