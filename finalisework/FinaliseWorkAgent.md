...
4. Verify proportionally through the supervisor: compact report first, then focused checks. Run `npm test` only when the route escalates to FULL, canonical verification is required, or the supervisor is unavailable/blocked. Run changed-file syntax checks where practical.
5. Recheck status. Stage only intentional changes; never stage generated runtime artifacts accidentally. `finalise work` authorizes committing and pushing intentional closeout changes to the current branch upstream after checks pass, unless the user says `no push`. Do not push on failed checks, no upstream, or unresolved unrelated work in the intended diff.
