...
- Before implementing any new durable state/file, classify its ownership as **project**, **user/app**, **session/runtime**, **cache**, or **developer/test**. If it is project-owned structured state or a project-owned file/asset, load `agents/PersistenceAgent.md` and follow `docs/architecture/project-storage-contract.md` before designing the save path.
